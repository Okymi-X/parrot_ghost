...
xterm*|rxvt*|tmux*)
    ...
    WIFI_DEV=$(nmcli -t -f DEVICE,TYPE connection show --active 2>/dev/null | grep ':802-11-wireless' | head -n1 | cut -d: -f1)
    ETHER_DEV=$(nmcli -t -f DEVICE,TYPE connection show --active 2>/dev/null | grep ':802-3-ethernet' | head -n1 | cut -d: -f1)
    VPN_DEV=$(ip -o -4 addr show | grep tun | awk '{print $2}' | head -n1)

    if [ -n "$WIFI_DEV" ]; then
      IP=$(ip -4 -o addr show $WIFI_DEV 2>/dev/null | awk '{print $4}' | cut -d/ -f1)
    elif [ -n "$ETHER_DEV" ]; then
      IP=$(ip -4 -o addr show $ETHER_DEV 2>/dev/null | awk '{print $4}' | cut -d/ -f1)
    elif [ -n "$VPN_DEV" ]; then
      IP=$(ip -4 -o addr show $VPN_DEV 2>/dev/null | awk '{print $4}' | cut -d/ -f1)
    else
